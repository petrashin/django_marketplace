...
class CartItems(models.Model):
    ...
    def get_user_or_session_id(self, request):
        ...
        if user.is_anonymous:
            ...
            if CartItems:
                ...
                cart_items = CartItems.objects.filter(user=0)
                if cart_items:
                    if len(cart_items) > 1:
    ...
    def get_cart_items(self, request):
        ...
        if request.user.is_authenticated:
            return CartItems.objects.filter(user=user)
        else:
    ...
    def add(self, request, product, quantity=1):
        """
    ...
    def get_total_cost(self, request):
        ...
        if cart_items:
            if len(cart_items) > 1:
                ...
                return cart_items[0].get_item_total_price()
            # else:
            #     cart_total_cost = Decimal('0.00')
